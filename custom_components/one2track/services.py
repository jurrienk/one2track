...
def _resolve_device(
    ...
    """Resolve entity IDs to (uuid, client, coordinator)."""
    if not entity_ids:
...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def handle_send_message(call: ServiceCall) -> None:
        uuid, client, _ = _resolve_device(hass, _extract_entity_ids(call))
    ...
    async def handle_force_update(call: ServiceCall) -> None:
        uuid, client, coordinator = _resolve_device(hass, _extract_entity_ids(call))
